...
resource "helm_release" "ocean-kubernetes-controller" {
  ...
  set = concat(
    ...
    var.image_pull_secrets != null ? [for idx, s in var.image_pull_secrets : {
      name  = "imagePullSecrets[${idx}].name"
      value = s
    }] : [],

    var.resources_limits != null ? [for k, v in var.resources_limits : {
      name  = "resources.limits.${k}"
      value = v
    }] : [],

    var.resources_requests != null ? [for k, v in var.resources_requests : {
      name  = "resources.requests.${k}"
      value = v
    }] : [],
